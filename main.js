...
// Big interior Door trigger
const bigInteriorDoorTrigger = new THREE.Box3(
  new THREE.Vector3(3.3, 0, 4.4),
  new THREE.Vector3(6.1, 2.8, 4.6)
)

// Corridor Door trigger
const corridorDoorTrigger = new THREE.Box3(
  new THREE.Vector3(3.3, 0, 4.4),
  new THREE.Vector3(6.1, 2.8, 4.5));
...
bigInteriorDoorTrigger.getSize(doorTriggerSize); 
...
doorTriggerMesh.position.copy(bigInteriorDoorTrigger.getCenter(new THREE.Vector3()));
...
function checkTriggers() {
  ...
  else if (bigInteriorDoorTrigger.containsPoint(camera.position) && currentRoom === 'reception') {
    loadCreepy();
  }
}
